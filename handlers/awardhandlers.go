...
func CreateAward(db *sql.DB) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// tex2pdf and return the string name of a file in /tmp.
		filename, err := award.Tex2Pdf()
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		//	get "filename" from /tmp generated by tex2pdf function.
		if err = award.EmailAward(filename); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
